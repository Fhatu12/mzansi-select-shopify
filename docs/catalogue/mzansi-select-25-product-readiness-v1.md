...
Slice **13M** (this pass, **docs-only**, pending commit): **`Neat Freak` removed from launch `Supplier verified` path** — **benchmark / category-catalogue reference only** — **not viable** **`Supplier verified` launch route** unless **written** resale/dropship/direct-fulfilment permission arrives **later** — **pause only** Neat Freak **`Supplier verified`** reliance, **not** product concepts. **Replacement routing:** **`ZA Dropshipping`** **primary** (Home/Kitchen/storage-style SKUs — Shopify integration, local-stock/no-inventory signal, fulfilment workflow, sample-order signal; **blocked** by missing exact app/admin refs, costs, shipping fee, delivery model, images, samples); **`Ecomstock`** **backup** (SA wholesale/retail categories; **blocked** by unclear dropship workflow, exact refs, fulfilment, shipping, images, samples); **`Gadgetgyz`** **preferred** for **`Cable Tidies Set`** and **Tech Accessories** (**stronger public dropship** than **Ecomstock** — **Cable Tidies** exact item still needed). **Product rows:** **`Sink Strainer`**, **`Compact Organiser`**, **`Mini Divider`** — **`ZA` first**, **`Ecomstock` backup** — **`Supplier proof in progress`**; **`Cable Tidies`** — **`Gadgetgyz` first**, **`ZA` backup** — **`Supplier proof in progress`**; **`Acrylic stand`** — **continue `Gadgetgyz`** (**Slice 13L**). **Collective replacement blockers:** exact replacement URLs/references; supplier/source confirmation; app/admin proof; current cost; shipping/delivery-cost model; delivery/SLA; stock reliability; image permission; sample/quality; return/refund practicality; **final margin after shipping** model; PO decision. **`Supplier verified` review:** **none** approved. **Next recommended slice:** **`Slice 13N`** — **`ZA Dropshipping`** replacement ref capture (**Gadgetgyz Cable Tidies** exact item optional in same pass **or separate**).
