...
**Prepared:** 2026-04-29  
**Owner:** Product Owner  
**Status:** Slice 13I controlled Shopify Admin preview-only listings now exist for five approved handles (`sink-strainer-stainless-steel`, `compact-organiser-basket`, `mini-plastic-divider-basket`, `cable-tidies-set`, `acrylic-tablet-phone-stand`) with Slice 13G safety tags and `0.00` placeholder pricing; Slice 13J records additional **theme-level** preview storefront safety (tag normalisation, zero-price + `Mzansi Select Preview` vendor guard, optional `image-permission-confirmed` gate for catalog media, cautious global copy on preview routes) as documented in the LLD — pending Product Owner commit decision; all five remain `Supplier proof in progress` only, not `Supplier verified`, not final-priced, not delivery- or claims-approved, not final launch approved; `Jewellery / accessory organiser` is not approved as final replacement; `Beauty & Hair` remains outside launch taxonomy  
**Version:** 2.0  
**Source of Truth:** `mzansi-select-theme.html`

## Purpose and scope

This document records the Slice 11F logged-in supplier verification checklist baseline, the Slice 11G.2 first-six supplier evidence capture outcome, the Slice 11H docs-only commercial gap-closure plan, the Slice 11I targeted gap-closure evidence pass, the Slice 12H docs-only collection readiness gap plan, the Slice 11K docs-only collection allocation and replacement-planning update, the Slice 11L replacement candidate supplier evidence pass, the Slice 11M replacement candidate commercial readiness plan, the Slice 11N Jewellery / accessory organiser supplier evidence closure, the Slice 11O Jewellery organiser viability and supplier-path decision, the Slice 11P Jewellery organiser DSers comparison and commercial assessment, the Slice 11Q Jewellery organiser current supplier proof and formal replacement evidence closure, the Slice 11R Jewellery organiser supplier proof capture status, the Slice 12I preview catalogue staging status, the Slice 12J preview product visibility foundation status, and the Slice 12K pause / Slice 13A local-first supplier pivot, Slice 13C shortlist, Slice 13D supplier proof-pack status, Slice 13E supplier proof gap-closure status, Slice 13F preview-staging preparation-pack status, Slice 13G preview-only safety-control status, and Slice 13I controlled Admin preview-only product listings for five approved handles, for the approved `25`-product launch catalogue.
...
Slice 13J (theme implementation, not yet git-committed unless Product Owner approves the commit) clarifies how the storefront should interpret preview readiness without Admin edits: use canonical tags `preview-only` and `price-to-confirm` (spacing variants are normalised in Liquid); treat non-positive variant price plus vendor `Mzansi Select Preview` as non-final pricing even if tags were mistyped; do not surface catalog product images for `preview-only` rows unless a separate `image-permission-confirmed` tag exists with Product Owner evidence; PDP delivery line remains exactly `Delivery details to be confirmed before launch.` Global announcement, trust-bar, and footer description switch to cautious copy only on preview-route contexts (see LLD). Adding `image-permission-confirmed` or changing vendor/tags in Admin remains a Product Owner / controlled-Admin decision, not implied by Slice 13J.
