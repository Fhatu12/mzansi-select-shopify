...
- **Pilot operational gates (Product Owner; tracker sync):** stock-failure/refund SLA, customer-update SLA, evidence retention/deletion (no customer/order PII in repo docs or committed artifacts; sanitized counts/statuses only in project docs), **R99** Gadgetgyz mechanism confirmation in a **separate bounded pass** before customer access, and **Admin/staging/checkout** deferred until after that confirmation + SOP controls are recorded — **canonical** in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** and **`docs/project-control.md` `## Slice 21J`**.
