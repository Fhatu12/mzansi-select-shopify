...
- **Slice 21X (PASS WITH NOTES):** **CJdropshipping** + **DSers** **replacement-route stack** for **new** **10–15** candidate international sourcing (**not** broad import; **not** app install until **Slice 21Y**) — **`## Slice 21X`**. **Temu** **benchmark-only** — **not** a supplier route. This **does not** remove the **local-first** posture for **South African** route proof in **`## Slice 20A`** / **`## Primary recommended suppliers`** — **21X** adds an **approved international replacement layer** alongside existing matrix rows.
...
- **Next gate:** **Slice 21Y** — **CJdropshipping** + **DSers** **app-permission / privacy risk review** **before** any **install** or **import**.
...
- **`CJdropshipping` / `DSers` (Slice 21X):** **approved** **replacement-route stack** for **new** international candidate sourcing — **`CJdropshipping`** **first**, **`DSers`** **second**, **hybrid** conditional; **no** Shopify app **install**/**import** until **`Slice 21Y`** — **`## Slice 21X`**.
