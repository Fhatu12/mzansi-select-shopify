...
- **Slice 21X (PASS WITH NOTES):** **CJdropshipping** + **DSers** **replacement-route stack** for **new** **10–15** candidate international sourcing (**not** broad import) — **`## Slice 21X`**. **Slice 21Y (PASS WITH NOTES):** **Security / Compliance** — **CJ** **route-first desk proof** **without** app install **may proceed**; **app install/import/sync** **remains BLOCKED** — **`## Slice 21Y`**. **Temu** **benchmark-only** — **not** a supplier route. This **does not** remove the **local-first** posture for **South African** route proof in **`## Slice 20A`** / **`## Primary recommended suppliers`** — **21X** adds an **approved international replacement layer** alongside existing matrix rows.
...
- **`CJdropshipping` / `DSers` (Slice 21X + 21Y):** **approved** **replacement-route stack** for **new** international candidate sourcing — **`CJdropshipping`** **first**, **`DSers`** **second**, **hybrid** conditional; **`Slice 21Y`** **Security / Compliance** **PASS WITH NOTES** — **CJ** desk proof **without** install **allowed**; **Shopify** **app install**/**import**/**sync** **remain BLOCKED** until **Product Owner** satisfies **Slice 21Y** **required controls** + an explicit later **install** slice — **`docs/project-control.md` `## Slice 21X`**, **`## Slice 21Y`**.
