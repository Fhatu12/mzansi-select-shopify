...
- Active slice: **Product Owner** — **Slice 21BG** north-star UI and interaction inventory reset is now recorded **PASS WITH NOTES** as a docs-only project reset. The reviewer lane is **paused**. The project is re-centred around **`mzansi-select-theme.html`** and **`mzansi-select-mobile.html`** as the approved north-star templates before more Shopify implementation work. **`docs/design/slice-21bg-north-star-ui-interaction-inventory.md`** now records the page structure, interactive elements, likely theme mapping, a theme-native Liquid/sections recommendation, and a small-slice implementation order that starts with visual parity and safe interactions only. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **customer account flows** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **product import/sync** **BLOCKED**; **app install** **BLOCKED**. Do **not** move to cart/checkout or purchase-flow enablement yet.
