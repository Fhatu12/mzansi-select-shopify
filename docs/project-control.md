...
- Active slice: **Slice 14C** (**next**) — wishlist honesty (**P0/P1**) — **Slice 14D** homepage **`featured-product-grid`** → PDP bridging **committed** **`fix(theme): link homepage cards to product pages`**
- Next supplier proof backlog (Product Owner sequencing): **PAUSED** until **Slice 14A.1** storefront **P0** **wishlist honesty** (**Slice 14C**) clears per PO — **Slice 14B** (**department routes**) and **Slice 14D** (**homepage product grids → `all_products` / `live-product-card`**) are **committed** in theme — then resume **`Slice 13N.2`**, **`Ecomstock` Sink Strainer** backup proof closure, **`Gadgetgyz` acrylic `Slice 13L` gates** — **`Cable Tidies Set`** **Slice 13O** documentation **committed** (`3bf260b830fcc4d4a55a7ed1314538ff748cb43f`); **product** proof closure **continues** when unpaused — **implementation sequencing (storefront):** **1.** **Slice 14B** — **committed** — **2.** **Slice 14D** — **committed** — **3.** **Slice 14C** — wishlist honesty — **4.** **Slice 14E** / **Slice 14G** (P1)
- Active owner: Product Owner
- Next owner: Senior Full-Stack Software Architect (**Slice 14C** implementation) with Product Manager (IA) / Product Owner (acceptance) — **supplier proof** resumes per PO when **Slice 14C** clears **wishlist P0**
...
- Last committed slice: **Slice 14D** **`fix(theme): link homepage cards to product pages`** — **`sections/featured-product-grid.liquid`**, **`templates/index.json`**, **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**, **`docs/project-control.md`** — prior **Slice 14B** (`1bef8f0c5658426860e35f49ef94ec86c3633110`, department collection routing + **`mzansi-select-theme.html`**)
- Last theme/code implementation slice: **Slice 14D** homepage **`featured-product-grid`** PDP bridging (**this pass**); prior **Slice 14B** (`1bef8f0c5658426860e35f49ef94ec86c3633110`); **Slice 13J** preview-only storefront safety (`cef5713412ef218bee4af56ae9767c78d6304859`); Slice 12J preview product visibility foundation (`263e60f1588b03f4120121007411c701d342d9e4`) remains earlier baseline
- Current blockers:
...
- Last tracker update: 2026-05-03 **Slice 14D** — homepage **`featured-product-grid`** → **`all_products`** / **`live-product-card`** when **Slice 13I** preview handles resolve (**`fix(theme): link homepage cards to product pages`**) — **`templates/index.json`** “View all” → **`/collections/all`** — **`docs/lld`** + **`project-control`** — **no** Shopify push/publish, **no** Admin edits — prior **Slice 14B** department URLs; **Slice 14A.1** evidence path unchanged under **`artifacts/`** (uncommitted)
- Tracker status: **Slice 14D** **committed** — **supplier proof:** **PAUSED** for **Slice 14C** wishlist **P0** — **`Slice 13N`** (**`421551…`**): **`ZA`** public refs **inconclusive** (unchanged); **`Ecomstock` Sink Strainer** (**`P5260S`**) backup retained; **`Slice 13O`**: **`Cable Tidies`** SKU **`PCB-CT-25150`** docs **committed** (**`3bf260b…`**) — **product-proof gates open** — **resume `Slice 13N.2` / `Ecomstock` sink / `Slice 13L` acrylic when PO unpauses supplier proof**
...
- Documentation sync status: **current for `project-control` + `docs/lld`** after **Slice 14D** (**this pass**); **`docs/catalogue`** **not edited**
- LLD status: **Updated Slice 14D** — documents **`featured-product-grid`** → **`all_products`** / **`live-product-card`** contract and **`templates/index.json`** section link defaults — **Slice 14C** wishlist behaviour **not** implemented **this pass**

## Current active pass

**Slice 14D** — **`fix(theme): link homepage cards to product pages`** — **`sections/featured-product-grid.liquid`**, **`templates/index.json`**, **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**, **`docs/project-control.md`**
...
- **Product Owner sequencing (post-sync):** **supplier-proof backlog paused** until **Slice 14C** wishlist **P0** clears — **Slice 14B** (**committed**) — **Slice 14D** homepage **`featured-product-grid`** (**committed**) — **Slice 14C** wishlist honesty **P0/P1** — **Slice 14E** / **14G** **P1**.
