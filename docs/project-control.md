...
- **Working copies:** Windows source retained at **`D:\dev\mzansi-select-shopify`** until **Product Owner** accepts migration parity; Ubuntu-Dev target at **`/home/fhatu/dev/mzansi-select-shopify`** (**Slice 21AI** **PASS WITH NOTES** — repo/git parity verified; **not** yet recommended as active working copy until Node.js is installed in Ubuntu-Dev and **Product Owner** accepts). **`artifacts/`** and **`zadropshipping/`** remain uncommitted (**`artifacts/`** not copied to Ubuntu; **`zadropshipping/`** remains Windows-only untracked).
...
- Completed slice: **Slice 21AI** — **Ubuntu-Dev** repo migration parity — **PASS WITH NOTES** — **DevOps / Platform Engineer** (**docs-only**; **no** Shopify Admin/theme/product/shipping/checkout/payment/customer-access/app install changes). **Method:** safe copy preserving **`.git`** (no usable remote). **Source:** **`D:\dev\mzansi-select-shopify`**; **target:** **`/home/fhatu/dev/mzansi-select-shopify`**. **Parity:** **master** @ **`211be9f297189de8332e28b0215f559a2b65efa8`**; **73** tracked files; **`docs/project-control.md`** + **`docs/pilot/`** present; **`artifacts/`** / **`zadropshipping/`** not copied (remain Windows local ignored/untracked). **Tool gaps:** Node.js **not** installed in Ubuntu-Dev; Shopify CLI **non-functional** until Node installed; Playwright **1.60.0** via **npx**. **Ubuntu-Dev active working copy:** **not recommended** until Node installed + **Product Owner** acceptance. **Report:** **`docs/migrations/mzansi-select-ubuntu-dev-migration-v1.md`**. **Evidence (Windows local; not committed):** **`artifacts/devops/slice-21ai-ubuntu-dev-migration-parity/20260515-113205/`**. **LLD:** **unchanged** — environment contract only; no theme/storefront/Admin behaviour change.
