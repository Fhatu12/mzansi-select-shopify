...
- Active slice: **Product Owner / User** — **bounded next work:** (1) approve a **separate bounded Shopify Admin shipping execution** slice when ready — **only** after **§12** + **§13** (**Slice 21N** execution-readiness) conditions and **§12.H** / **§6B** gates (**`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §12–§13** + **`## Slice 21M`**, **`## Slice 21N`**); (2) close remaining **Slice 21K** basket/setup questions where still open; (3) confirm **four pilot** Shopify product rows vs **Slice 21J** SKUs before product-specific shipping profile execution; continue **controlled Slice 15A-style feedback-only** private preview sharing where applicable (**Slice 19B PASS** — **wider private-preview sharing** pause **lifted** from **QA/readiness** perspective only; **no** public launch, checkout/payment testing, final pricing, delivery promises, product claims, **Supplier verified**, product import, Shopify Admin edits, or customer account / wishlist / newsletter / social **service** enablement — see **`## Slice 19B`**)
...
- Completed slice: **Slice 21N** — Shopify pilot shipping **configuration execution-readiness** check (DevOps / Platform Engineer; **docs only**; **PASS WITH NOTES**; **no** Admin execution; **no** shipping configuration; **CONDITIONAL GO** for a **later** bounded execution slice per **pilot §13**; **customer access remains BLOCKED**; **Admin execution** remains **BLOCKED** until **separate PO-approved** execution slice) — see **`## Slice 21N`**
...
- Documentation sync status: **current for `project-control` + pilot doc (§12 Slice 21M; §13 Slice 21N) + catalogue Slice 21J / 21K / 21L + LLD Slice 21I–21M pilot delivery / checkout planning notes** after **Slice 21N** tracker sync; prior **Slice 19B** sync remains valid for preview-sharing posture
...
- **Product Owner operational decisions (pilot):**
  ...
  5. **Admin / staging / checkout:** **not approved** yet; requires a **separate bounded execution slice** after **Product Owner** accepts **Slice 21M** (**pilot §12**), **Slice 21N** readiness conditions (**pilot §13.9**), **§6A**/**§6B** gates, and **Product Owner** go/no-go (**Slice 21L** / **Slice 21M** / **Slice 21N** do **not** configure **Shopify**).
...
- **Recommended next owner:** **Product Owner / User** — **accept Slice 21M** (**pilot §12**) and **Slice 21N** (**pilot §13**) then confirm **§13.9** checklist before approving a **bounded Admin shipping execution** slice; **DevOps** / implementation owner — **bounded Shopify** shipping/checkout **Admin configuration** only after explicit written approval (**plan:** pilot **§6A** + **§12** + **§13**); then **Security / Compliance** + **QA** as named in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`**.
...
- **Bounded Shopify shipping/checkout planning:** short gate in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §6A**; full DevOps plan in **§12** (**Slice 21M**); **execution-readiness** in **§13** (**Slice 21N**) — **do not configure** until **Product Owner** accepts **§12**, **§13.9** pre-execution confirmations, and a separate approved **execution** slice; required future checks and **§6B** customer-access blockers are **canonical** in the pilot doc.
...
- **Canonical detail:** **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** (**§1**, **§5**, **§6A**, **§6B**, **§12**, **§13**).
- **Recommended next owner:** **Product Owner / User** — **accept** **§12** + **§13** then go/no-go for a future **bounded execution** slice; **DevOps** / implementation owner — execute Admin shipping/checkout configuration **only** after that written approval + **§13.9** confirmations.
...
- **Canonical operational plan:** **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §12** — verdict; Shopify Admin areas for a later execution slice; recommended pilot shipping-rate setup and options evaluation; interpretation of **Pilot courier delivery — R89**; checkout/payment readiness checks; rollback; QA validation (later execution only); Product Owner approvals; risks/blockers. **Execution-readiness:** **§13** (**Slice 21N**).
...
- **Recommended next owner:** **Product Owner / User** — review and **accept or revise** **§12** + **§13** before any DevOps Admin execution slice is scheduled.

## Slice 21N Shopify pilot shipping configuration execution-readiness check (DevOps / Platform Engineer; **docs only**)

- **Decision:** Product Owner accepts **Slice 21N** as **PASS WITH NOTES** — **execution-readiness check only**; **no** Shopify Admin shipping configuration; **no** checkout/payment/customer-access enablement.
- **Prerequisite:** **Slice 21M** planning commit **`3f791a9fc7d4614731030188073eeb6c66318969`**; refines **§12** with feasibility and go/no-go framing.
- **Customer access:** **BLOCKED** (unchanged).
- **Admin / configuration execution:** **BLOCKED** until **Product Owner** separately approves a **later bounded execution** slice consistent with **pilot §13.9**.
- **Go / no-go (for a future execution slice):** **CONDITIONAL GO** — see **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §13.9** (pilot products/handles, profile strategy, market scope, **Pilot courier delivery — R89** visibility, payment/capture posture, access gate, **§12.H** approvals).
- **Isolation posture:** **Preferred:** pilot-only / product-specific shipping profile **after** four pilot Shopify rows confirmed; **Fallback:** general-profile manual rate **only** with explicit PO risk acceptance + strict password/invite-only containment (**pilot §13.2–§13.5**).
- **Product prerequisite:** product-isolated shipping cannot be **proven** in QA until pilot Admin rows exist and profile mapping is documented — **catalogue** cross-check **Slice 21J** vs historical **Slice 13I** five-handle set (**pilot §13.3**).
- **Non-approvals preserved:** no product import/staging in **Slice 21N**; no theme push/publish; **do not** commit **`zadropshipping/`** or **`artifacts/`**; no secrets or live customer/order/payment data in repo.
- **Canonical detail:** **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §13**.
- **Recommended next owner:** **Product Owner / User** — confirm **§13.9** checklist, then approve **bounded Admin shipping execution** slice when ready; **DevOps** executes **only** after that approval.
