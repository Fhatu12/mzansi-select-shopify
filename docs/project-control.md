...
- Active slice: **Slice 17I** — authenticated preview regression for search select honesty (**accepted tracker sync this pass**)
- Next implementation slice: **Slice 17J** — search page body preview-commerce honesty remediation
...
- Next owner: **Product Owner** to approve Slice **17J** after accepted Slice **17I** QA findings
- Last accepted slice: **Slice 17I** authenticated preview regression for search-select honesty accepted **PASS WITH NOTES**; **Slice 17F** search select honesty decision remains accepted **PASS WITH NOTES**; **Slice 17A** HTML interaction contract audit acceptance remains recorded canonically under the original evidence label **`Slice 16B — HTML foundation and interactive UI contract audit`**, and **Slice 16D** HTML foundation remediation verification remains accepted **PASS WITH NOTES**
- Last committed slice: **Slice 17G** desktop search select honesty — removes the misleading active-looking desktop category/select state while preserving the working search route and leaving mobile search unchanged
- Last theme/code implementation slice: **Slice 17G** desktop search select honesty (**this pass**) — the desktop header now keeps `routes.search_url`, the `q` input, and search submit intact while the non-wired category/select becomes an honest deferred preview control; no predictive search, Search & Discovery facets, collection-scoped search, or new filtering service is introduced
- Current blockers:
  ...
  - **Separate follow-up from Slice 17I:** the **`/search`** page body still renders the older static preview-search foundation with placeholder commerce cards and visible **Add to Cart** buttons. This is recorded as a **pre-existing search-surface honesty gap**, not a **Slice 17G** header/select regression, and should be handled as **Slice 17J — Search page body preview-commerce honesty remediation**.
...
- Last tracker update: 2026-05-10 **QA acceptance + tracker sync** — **Slice 17I** authenticated preview regression accepted **PASS WITH NOTES**; the desktop deferred search select contract is validated in real unpublished preview, while the **`/search`** body gap is split into follow-up **Slice 17J**
- Tracker status: **Slice 17I** accepted **PASS WITH NOTES** — **Slice 14B / 14C / 14D**, **Slice 17B**, **Slice 17C**, **Slice 17E**, **Slice 17G**, and PDP preview-only safety remain intact; the desktop category/select no longer presents as an active unwired filter, authenticated preview validation is restored, and the next implementation follow-up is **Slice 17J** for the separate search-page-body honesty gap
...
- Documentation sync status: **current for `project-control`** after **Slice 17G** implementation sync
- LLD status: **updated in Slice 17G** for final technical sync because the accepted implementation now needs an explicit implementation note and the later slice numbering must stay consistent after reusing **`17G`** for the bounded desktop search-select change

## Current active pass

**Documentation + tracker sync (this pass):** **Slice 17I** — record the accepted authenticated preview regression verdict, validated preview evidence, and the separate **Slice 17J** follow-up for the search page body honesty gap.

## Slice 17I authenticated preview regression for search select honesty

- **Decision:** Product Owner accepted **Slice 17I** as **PASS WITH NOTES**.
- **Evidence folder:** **`artifacts/qa/slice-17i-authenticated-preview-search-select-regression-rerun/20260510-142913/`**.
- **Validated preview theme:** **`151207542967 / Mzansi Select MVP Preview / unpublished`**.
- **Validated routes:** **`https://dropshippoc.myshopify.com/?preview_theme_id=151207542967`**, **`https://dropshippoc.myshopify.com/search?preview_theme_id=151207542967`**, and **`https://dropshippoc.myshopify.com/search?q=strainer&type=product&preview_theme_id=151207542967`**.
- **Accepted QA results:** **Desktop search submit: PASS WITH NOTES**; **type=product preservation: PASS**; **Disabled/deferred select honesty: PASS**; **Mobile search regression: PASS WITH NOTES**; **No predictive search, facets, collection-scoped search, or new filtering service appeared: PASS**; **Header/layout visual regression: no obvious desktop or mobile header regression found**.
- **Accepted interpretation:** the real unpublished preview storefront now validates the **Slice 17G** desktop header/select honesty change, the working Shopify search route, preserved **`type=product`** submission, and unchanged mobile header search path.
- **Separate follow-up issue:** the **`/search`** page body still renders the older static preview-search foundation with placeholder commerce cards and visible **Add to Cart** buttons. This is a **pre-existing search-surface honesty gap**, not a regression introduced by **Slice 17G**.
- **Recommended next slice:** **Slice 17J — Search page body preview-commerce honesty remediation**.
- **Recommended next owner:** **Product Owner** to approve **Slice 17J**.
