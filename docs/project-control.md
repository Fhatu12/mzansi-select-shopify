...
- Active slice: **Product Owner / User** — **bounded next pass:** confirm **Gadgetgyz** pilot **basket composition** (whether **R745.70** total reflected exactly the four approved pilot SKUs), **fee sensitivity** (destination, quantity, mixed basket), **free-shipping threshold** visibility, and **pre-payment / no-order** fee visibility — **separate from customer access**; continue **controlled Slice 15A-style feedback-only** private preview sharing where applicable (**Slice 19B PASS** — **wider private-preview sharing** pause **lifted** from **QA/readiness** perspective only; **no** public launch, checkout/payment testing, final pricing, delivery promises, product claims, **Supplier verified**, product import, Shopify Admin edits, or customer account / wishlist / newsletter / social **service** enablement — see **`## Slice 19B`**)
...
- Documentation sync status: **current for `project-control` + pilot doc + catalogue Slice 21J / 21K + LLD pilot delivery notes** after **Slice 21K** partial shipping tracker sync; prior **Slice 19B** sync remains valid for preview-sharing posture
...
- **Approved controlled pilot planning baseline (summary):**
  ...
  - **Pilot delivery (planning):** historical docs used a **R99** flat pilot delivery **assumption** — **no longer accepted as confirmed** (**Slice 21K**). Current planning signal: **Gadgetgyz** supplier-checkout observation — **Courier Door to Door Delivery — Economy**, **R89.00**, checkout-displayed **3 to 5 business days** (**supplier checkout signal only**); basket total shown **R745.70** — see **`## Slice 21K`**. **Do not** treat **R89** or timing as final customer-facing delivery promise until basket mix and setup are confirmed. **R99** should **not** be reintroduced in pilot planning unless later supplier evidence explains a different scenario.
...
- **Product Owner operational decisions (pilot):**
  ...
  4. **Gadgetgyz delivery/shipping mechanism:** **Slice 21K** records **partial** supplier-checkout evidence (**R89** Economy, **3–5 business days** as displayed at checkout, basket **R745.70**) — **not** a final customer-facing delivery promise; prior **R99** flat assumption **withdrawn as confirmed**. Further confirmation (exact four-SKU basket, destination/quantity/mixed-basket behaviour, free-shipping threshold, pre-payment visibility) remains **before** customer access. **No** customer-facing delivery policy until verified.
  5. **Admin / staging / checkout:** **not approved** yet; requires a **separate bounded slice** after shipping evidence + SOP controls are recorded (**Slice 21K** does **not** by itself approve Admin/checkout).
- **Customer access:** **BLOCKED** until a separate explicit go/no-go (unchanged).
- **Still blocked (explicit):** customer access; Shopify Admin edits; product import; product staging; payment/checkout enablement; theme push/publish/live overwrite; **Supplier verified** promotion; final pricing approval; delivery promise approval; product claim approval; public launch approval.
...
- **Recommended next owner:** **Product Owner / User** — close **Slice 21K** open items (exact four-SKU basket vs **R745.70**, fee sensitivity, free-shipping threshold, pre-payment fee visibility) — **or** **Product Manager** — update pilot delivery **recommendation** (docs only); then **Security / Compliance** + **DevOps** + **QA** when a later bounded implementation or Admin slice is opened **after** explicit go/no-go.
...
- **Customer-facing posture:** keep **§1 approved pilot delivery wording** in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** (conservative, **not guaranteed**) until exact pilot basket/product mix and delivery setup are confirmed.
