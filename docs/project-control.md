...
- Active slice: **Product Owner** — **Slice 21BS** restricted-preview user guide is now recorded as source-truth-preserving internal documentation for the current north-star reset / restricted-preview lane. **Slice 21BQ** bounded browse/link specificity and route-honesty implementation remains in local theme code **PASS WITH NOTES** pending preview-side validation, so the new guide labels any such behaviour as a **current preview expectation** or **to be confirmed** where appropriate. The project remains re-centred around **`mzansi-select-theme.html`** and **`mzansi-select-mobile.html`**. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **customer account flows** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **product import/sync** **BLOCKED**; **app install** **BLOCKED**. Do **not** move to cart/checkout or purchase-flow enablement yet.
