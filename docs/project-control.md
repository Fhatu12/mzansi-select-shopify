...
- Active slice: **Slice 14D** (**next**) — homepage merchandising card-to-PDP bridging — **Slice 14B** launch department collection routing **committed** **`fix(theme): route launch departments to collections`**
- Next supplier proof backlog (Product Owner sequencing): **PAUSED** until **remaining** **Slice 14A.1** storefront **P0** items clear (**Slice 14D** homepage static cards, **Slice 14C** wishlist honesty, plus documented follow-ups) — **Slice 14B** addresses **department hash / wrong-destination P0** in theme sources — then resume **`Slice 13N.2`**, **`Ecomstock` Sink Strainer** backup proof closure, **`Gadgetgyz` acrylic `Slice 13L` gates** — **`Cable Tidies Set`** **Slice 13O** documentation **committed** (`3bf260b830fcc4d4a55a7ed1314538ff748cb43f`); **product** proof closure **continues** when unpaused — **implementation sequencing (storefront):** **1.** **Slice 14B** — **committed** (**this pass**) — **2.** **Slice 14D** — homepage card-to-PDP bridging — **3.** **Slice 14C** — wishlist honesty (P0/P1) — **4.** **Slice 14E** / **Slice 14G** (P1)
- Active owner: Product Owner
- Next owner: Senior Full-Stack Software Architect (**Slice 14D** next) with Product Manager (IA) / Product Owner (acceptance) — **supplier proof** resumes per PO when **remaining P0 storefront** gates clear
- Last accepted slice: **`Slice 14A.1`** unlocked storefront interaction audit **PASS WITH NOTES** (headed Playwright after manual storefront unlock; evidence **`artifacts/qa/slice-14a1-unlocked-storefront-interaction-audit/20260503T170500Z/`**); prior supplier-proof acceptance: **`Slice 13O`** **`Gadgetgyz` `Cable Tidies Set`** proof **accepted**, **committed**, **closed** (`3bf260b830fcc4d4a55a7ed1314538ff748cb43f`); **Slice 13N.1** tracker sync (`f10eedcaa2dd6e6b20967960bb5bebed5002e38a`); **Slice 13N** (`421a5514cf33430e633763741c7a0e4bf054a969`); Slice 13M (`45eb8e23b568e8fc7f6bdd2413818caa29288e5e`); Slice 13L.1 (`a33895a3bb242455c95da74a6c6f2c72bee946d6`); Slice 13L (`a87e8c6124d9708967dfbf2c6a208b601fe6f608`); Slice 13K.1 (`0054d66acfe926be21127ae02ba7453989c11131`); Slice 13K (`cb61ee28fbb3e575669236310d4e5b131fc697bf`); Slice 13J (**PASS WITH NOTES**)
- Last committed slice: **Slice 14B** **`fix(theme): route launch departments to collections`** — **`snippets/launch-collection-url.liquid`**, **`sections/primary-navigation.liquid`**, **`sections/category-strip.liquid`**, **`sections/main-search-foundation.liquid`**, **`mzansi-select-theme.html`**, **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**, **`docs/project-control.md`** — prior **Slice 14A.1** (`168bb343cf7e00a68fbcc10abe973869e8798555`, **`docs/project-control.md` only**)
- Last theme/code implementation slice: **Slice 14B** launch department collection routing (**this pass**); prior **Slice 13J** preview-only storefront safety (`cef5713412ef218bee4af56ae9767c78d6304859`); Slice 12J preview product visibility foundation (`263e60f1588b03f4120121007411c701d342d9e4`) remains earlier baseline
- Current blockers:
  - The five approved preview-only products now exist in Shopify Admin with Slice 13G safety tags and placeholder pricing, are published to **Online Store** for preview review only, and remain `Supplier proof in progress` only — not `Supplier verified`, not import-approved, not final-priced, and not launch-approved; unauthenticated preview-theme URL checks still hit the storefront password wall, so visual PDP/collection confirmation generally requires an authenticated preview session — **Slice 14A.1** (**PASS WITH NOTES**, headed Playwright after manual unlock) **did** exercise themed UI on required desktop routes + mobile subset (**`/`**, **`/collections/all`**, **`/products/cable-tidies-set`**) with preview theme **`151207542967`**
  - **Storefront UX gates (Slice 14A.1 baseline — carry-forward):** **(P0)** favourite/heart controls (`.p-wish`) are **not** real favourite controls — **no** `aria-pressed` change, **no** class change signalling selection, **no proven** wishlist **`localStorage` / `sessionStorage` contract** — header **`#wishlist`** remains a **non-route** placeholder; **(P0)** homepage **static merchandising** product cards **do not navigate** to PDP from card interactions (**Slice 14D**); **(follow-up)** observed web-storage deltas were **not proven** wishlist-specific; **keyboard Tab order** and **collection PDP mapping** (which PDP a grid row opens) need **follow-up** human/QA review — **supplier-proof backlog stays paused** until **remaining** **P0 storefront** items (**14D**, **14C**) clear per PO sequencing — **Slice 14B** **committed** routes the approved launch quartet to **`collections['handle'].url`** (**`snippets/launch-collection-url.liquid`**, **`all-products` fallback** if handle missing) in **`primary-navigation`**, **`category-strip`**, **`main-search-foundation`**; **`mzansi-select-theme.html`** uses **`/collections/{handle}`** — **historical QA** **`#home-living`** finding addressed at source (**no** **`#home-living`**-style launch department **`href`** in those surfaces)
...
- Last tracker update: 2026-05-03 **Slice 14B** — launch department **collection URLs** wired in theme (**`fix(theme): route launch departments to collections`**) — quartet handles **`home-living`**, **`kitchen-storage`**, **`office-desk`**, **`tech-accessories`** — **`docs/lld`** updated — **no** Shopify push/publish, **no** Admin edits — same-day prior: **Slice 14A.1** **PASS WITH NOTES** evidence **`artifacts/qa/slice-14a1-unlocked-storefront-interaction-audit/20260503T170500Z/`**; **Slice 13O** **`3bf260b…`**
- Tracker status: **Slice 14B** **committed** — **supplier proof:** **PAUSED** for **remaining P0 storefront** (**14D** / **14C**) — **`Slice 13N`** (**`421551…`**): **`ZA`** public refs **inconclusive** (unchanged); **`Ecomstock` Sink Strainer** (**`P5260S`**) backup retained; **`Slice 13O`**: **`Cable Tidies`** SKU **`PCB-CT-25150`** docs **committed** (**`3bf260b…`**) — **product-proof gates open** — **resume `Slice 13N.2` / `Ecomstock` sink / `Slice 13L` acrylic when PO unpauses supplier proof**
...
- Documentation sync status: **current for `project-control` + `docs/lld`** after **Slice 14B** (**this pass**); **`docs/catalogue`** **not edited**
- LLD status: **Updated Slice 14B** — documents **`launch-collection-url`** contract, **`primary-navigation` / `category-strip` / `main-search-foundation`** wiring, and **`mzansi-select-theme.html`** parity — **Slice 14C / 14D** behaviour **not** implemented **this pass**

## Current active pass

**Slice 14B** — **`fix(theme): route launch departments to collections`** — **`snippets/launch-collection-url.liquid`**, **`sections/primary-navigation.liquid`**, **`sections/category-strip.liquid`**, **`sections/main-search-foundation.liquid`**, **`mzansi-select-theme.html`**, **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**, **`docs/project-control.md`**
...
- **Accepted P0/P1 issue record:**
  1. **Favourite/heart controls** are **not** real favourite controls — **no** `aria-pressed` change; **no** class change; **no proven** wishlist **`localStorage` / `sessionStorage` contract**; header **`#wishlist`** unchanged.
  ...
  3. **Homepage static merchandising** cards **do not navigate** — **P0**.
...
- **Product Owner sequencing (post-sync):** **supplier-proof backlog paused** until **remaining P0 storefront** issues resolve — **Slice 14B** (**committed**) — **department collection routing** — **next:** **Slice 14D** homepage **card→PDP** — **Slice 14C** wishlist honesty **P0/P1** — **Slice 14E** / **14G** **P1**.
...
- **Disclosures:** **no** wishlist (**14C**), **no** homepage merchandising card PDP wiring (**14D**), **no** **`docs/catalogue`** edits, **no** **`artifacts/`** commit, **no** Shopify push/publish, **no** Admin product/collection/checkout changes, **no** **`Supplier verified`** / launch / pricing / delivery / claim approvals.
