...
- Active slice: **QA / Test Engineer** — rerun **Slice 21Q-B3** homepage trust-chrome regression after residual desktop-nav fulfilment wording remediation; the latest homepage-only QA rerun **FAIL** narrowed the remaining issue to visible homepage desktop-nav badge copy **`Fast Delivery South Africa Wide`** on `/` at **1366x768** even after the earlier category-strip fix left **`Help before fulfilment →`** live and **`Easy returns`** absent; root cause is the desktop badge literals in **`sections/primary-navigation.liquid`**; local remediation now rephrases those nav badges to **`Delivery review`** and **`South Africa only`** without changing category-strip wording, topbar/trust-bar wording, layout structure, or any Shopify/Admin/product/shipping/payment/customer-access state; **no** Shopify Admin/import/staging/shipping/payment/customer-access/theme publish action occurred in this remediation pass; recommended next owner remains **QA / Test Engineer**
