...
- Active slice: **Slice 14E** / **Slice 14G** (**P1** follow-ups) or **supplier-proof** backlog — **Slice 14C** wishlist honesty **committed** **`fix(theme): make wishlist controls honest`**
- Next supplier proof backlog (Product Owner sequencing): **Slice 14A.1** storefront navigation **P0** theme passes **14B**, **14D**, **14C** are **committed** — **supplier-proof backlog may resume per Product Owner** (not automatic) — **`Slice 13N.2`**, **`Ecomstock` Sink Strainer** backup proof closure, **`Gadgetgyz` acrylic `Slice 13L` gates`** — **`Cable Tidies Set`** **Slice 13O** (`3bf260b830fcc4d4a55a7ed1314538ff748cb43f`) — **implementation sequencing (storefront):** **1.** **14B** **2.** **14D** **3.** **14C** — **all committed** — **4.** **14E** / **14G** (**P1**)
- Active owner: Product Owner
- Next owner: Product Owner — **unpause supplier proof** when ready; **Senior Full-Stack Software Architect** for **Slice 14E** / **14G** if PO prioritises storefront **P1**
...
- Current blockers:
  ...
  - **Storefront UX gates (Slice 14A.1 baseline — carry-forward):** **(Slice 14C — committed)** hearts **`.p-wish`** are **`disabled`** + **`aria-disabled="true"`** with **honest** **`aria-label`** copy (**no** save-to-wishlist pretence); header wishlist is **`<span class="hdr-action hdr-action--deferred">`** (**no** **`#wishlist`** **hash** route); footer wishlist is **non-link** **`f-link-deferred`**; PDP **`Wishlist deferred`** button gains **`aria-disabled`** + **`aria-label`**; **CSS** supports deferred/disabled heart/header/footer — **no** `aria-pressed`, **no** fake toggle classes, **no** wishlist **`localStorage` / `sessionStorage`** — **(follow-up)** keyboard **Tab** order and **collection PDP mapping** still merit human/QA review — **Slice 14B** / **14D** theme commitments unchanged (**department URLs**, **`featured-product-grid`** → **`live-product-card`**)
...
- Last tracker update: 2026-05-03 **Slice 14C** — wishlist / heart controls **honest deferred** state (**`fix(theme): make wishlist controls honest`**) — **`static` + `live` product cards`**, **header**, **footer**, **PDP**, **`theme.css`** — **`docs/lld`** + **`project-control`** — **no** Shopify push/publish, **no** Admin edits — prior **Slice 14D** homepage grids + **Slice 14B** departments
- Tracker status: **Slice 14C** **committed** — **supplier proof:** **eligible to resume per Product Owner** — **Slice 14A.1** storefront navigation theme **P0** items **14B / 14D / 14C** **committed** — **`Slice 13N`** (**`421551…`**): **`ZA`** public refs **inconclusive** (unchanged); **`Ecomstock` Sink Strainer** (**`P5260S`**) backup retained; **`Slice 13O`**: **`Cable Tidies`** SKU **`PCB-CT-25150`** docs **committed** (**`3bf260b…`**) — **product-proof gates open**
...
- Documentation sync status: **current for `project-control` + `docs/lld`** after **Slice 14C** (**this pass**); **`docs/catalogue`** **not edited**
- LLD status: **Updated Slice 14C** — documents deferred wishlist **UI contract** (**header span**, **footer span**, **`.p-wish` disabled**, **PDP** **`aria-label`**) — **Slice 14E** / **14G** **not** implemented **this pass**
...
- **Product Owner sequencing (post-sync):** **Slice 14B** / **14D** / **14C** storefront navigation theme passes **committed** — **supplier-proof backlog** **may resume per Product Owner** — **Slice 14E** / **14G** **P1** follow-ups remain.
