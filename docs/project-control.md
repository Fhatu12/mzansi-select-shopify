...
- Active slice: **Product Owner / User** — approve **bounded Shopify Admin product-staging execution** slice when ready (**`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §15** + **`## Slice 21P`**); **Product Manager** or **Senior Full-Stack Software Architect** — maintain **§15** as execution spec until slice completes; **gates unchanged:** **no** Admin execution from **Slice 21P** docs alone; **verify existing Shopify rows** before writes; **reuse/update only** after **separate PO** approval per SKU; **no duplicate rows**; **product-specific shipping profile only**; **general-profile fallback blocked**; **South Africa only**; **Pilot courier delivery — R89** **conditional** / **controlled checkout testing only**; **R89** supplier re-check before access; **payment/capture blocked**; **customer access BLOCKED**; **no** manual card data; continue **Slice 19B** bounded preview posture (**`## Slice 19B`**)
...
- Last tracker update: **Slice 21P** — bounded **Shopify Admin product-staging implementation plan** recorded (**docs only**; **pilot §15**); **Slice 21O** — **PASS WITH NOTES** (readiness **§14** closed by **§15**); approved **intended handles** (`acrylic-tablet-or-phone-stand`, `ugreen-4-in-1-usb-2-0-hub`, `gizzu-usb-to-type-c-cable-2m`, `world-map-extended-mouse-pad`); **Controlled Pilot** / `controlled-pilot`; tags **pilot**, **pilot-gadgetgyz**, **pilot-21**, **supplier-gadgetgyz**, **manual-fulfilment**; **Default Title** + SKU mapping; pilot prices **R249** / **R279** / **R129** / **R219**; **verify existing rows** before execution; **reuse/update only** after **separate PO** approval; **no duplicates**; **product-specific shipping only**; **general-profile blocked**; **no** Admin/staging/import/shipping/checkout/payment/customer-access execution in this pass; prior **Slice 21N** / **21M** / **21L** / **21K** / **21J**; **Slice 19B** preview posture unchanged
- Tracker status: **Slice 21P** (**planning recorded**; **§15**; **no** Admin) + **Slice 21O PASS WITH NOTES** (**§14**) + **Slice 21N PASS WITH NOTES** (**§13**) + **Slice 21M** (**§12**) + **Slice 21L PASS WITH NOTES** + **Slice 21K PASS WITH NOTES** + **Slice 21J PASS WITH NOTES** + **Slice 19B PASS** + **Slice 19A PASS WITH NOTES** + **Slice 18F** … **18A–C** — storefront **`/search`** honesty intact — **customer access BLOCKED** — **Shopify** shipping **unconfigured**
...
- Documentation sync status: **current for `project-control` + pilot doc (§12–§15) + catalogue Slice 21J / 21O / 21P + LLD** after **Slice 21P**; prior **Slice 19B** preview-sharing posture unchanged
