...
- Active slice: **QA / Test Engineer** — rerun **Slice 21Q-B3** trust-chrome storefront regression after local homepage category-strip remediation; the prior QA rerun **FAIL** was narrowed to residual homepage visible copy **`Returns Easy returns →`** on `/` across **390x844**, **768x1024**, and **1366x768`; root cause is the literal sub-copy in **`sections/category-strip.liquid`**; local remediation now rephrases that deferred tile to **`Help before fulfilment →`** without changing topbar/trust-bar wording, layout structure, or any Shopify/Admin/product/shipping/payment/customer-access state; **no** Shopify Admin/import/staging/shipping/payment/customer-access/theme publish action occurred in this remediation pass; recommended next owner remains **QA / Test Engineer**
