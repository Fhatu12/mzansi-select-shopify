...
- Active slice: **Product Owner** — **Slice 21BU** restricted-preview application walkthrough guide is now recorded as a click-through companion to the accepted **Slice 21BS** general user guide. The walkthrough keeps the current north-star reset / restricted-preview lane practical for internal reviewers, while preserving the same source-truth posture. **Slice 21BQ** bounded browse/link specificity and route-honesty implementation remains **PASS WITH NOTES** pending preview-side rendered confirmation where not already covered by accepted evidence, so the walkthrough also labels such behaviour as a **current preview expectation** or **to be confirmed** where appropriate. **PDF export is paused until this walkthrough guide is accepted.** The project remains re-centred around **`mzansi-select-theme.html`** and **`mzansi-select-mobile.html`**. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **customer account flows** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **product import/sync** **BLOCKED**; **app install** **BLOCKED**. Do **not** move to cart/checkout or purchase-flow enablement yet.
