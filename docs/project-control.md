...
- Active slice: **Slice 21T** — controlled pilot **product-specific shipping setup execution gate** (**DevOps / Platform Engineer**): bounded Shopify Admin shipping profile execution completed for exactly four locked **Gadgetgyz** draft pilot variants under **`Mzansi Select Controlled Pilot — Gadgetgyz`** with **South Africa** (**ZA**) zone and active **`Pilot courier delivery`** fixed rate at **ZAR 89.0**; all four pilot variants moved off **`General profile`** into the isolated profile; **`General profile`** remains in place with **South Africa** (**ZA**) and active **`Standard shipping`** at **ZAR 99.0** for other products. **No** checkout/payment/customer-access/public-launch enablement; **no** theme push/publish; **no** product data change beyond shipping-profile assignment. **Next operational gate:** bounded checkout/payment readiness + QA while customer access remains blocked (see **Next owner**). Continue **Slice 19B** bounded private-preview posture (**`## Slice 19B`**)
...
- Active owner: **Product Owner / User** — **controlled Slice 15A-style feedback-only** private preview sharing (**wider private-preview sharing** QA/readiness pause **lifted** per **Slice 19B PASS** — see **`## Slice 19B`**)
- Next owner: **Product Owner / User** — approve or defer a **later bounded** **Shopify Admin** **shipping execution** slice (**pilot-only / product-specific** profile strategy, **product ↔ profile** assignment, **exact rate label/amount**, **rollback**, **QA validation** path). **DevOps / Platform Engineer** — execute **only** after that **written** **Product Owner** approval (**§12.H** / **`docs/pilot/...` §15B**). **Until then:** **shipping execution** remains **BLOCKED**. **Do not** commit **`zadropshipping/`** or **`artifacts/`**.
- Last accepted slice: **Slice 21S** — controlled pilot shipping-configuration **readiness/execution pass** — **PASS WITH NOTES** — **readiness-only** — **Product Owner accepted** — docs commit **`3d1511b`** (`docs: record controlled pilot shipping readiness`); evidence **`artifacts/devops/slice-21s-controlled-pilot-shipping-configuration-readiness/20260512-205422/`** — **no** Admin shipping change. **Prior accepted anchors:** **Slice 21R** (product-row lock); **Slice 21Q-B3**; **Slice 21O** (**§14**); **Slice 21N** — commit **`417e78f5ec37ab8d9e450a8fd7d9aab9c10025fd`** (**§13**); **Slice 21M** (**§12**); **Slice 21L** / **21K** / **21J**; **Slice 19B PASS** and earlier tracker chain unchanged.
...
- Current blockers:
  - **Controlled-pilot headline gates:** **customer access** **BLOCKED**; **shipping Admin execution** **BLOCKED** until **Product Owner** approves **product-specific** shipping-profile strategy, **product ↔ profile** assignment scope, **exact rate label/amount**, **rollback** path, and **QA validation** path; **checkout / payment** **BLOCKED**; **public launch** **BLOCKED**; **`Supplier verified`**, **final pricing**, **final delivery promises**, and **final product claims** **BLOCKED**.
...
- Last tracker update: **Slice 21T** — controlled pilot **product-specific shipping setup execution gate** — **PASS WITH NOTES**. Evidence: **`artifacts/devops/slice-21t-controlled-pilot-product-specific-shipping-setup/20260513-221127/`**. Records the bounded Admin shipping change from **`General profile`** only to a two-profile state: default **`General profile`** still present with **South Africa** (**ZA**) and **`Standard shipping`** **ZAR 99.0**, plus **`Mzansi Select Controlled Pilot — Gadgetgyz`** with **Shop location**, **South Africa** (**ZA**), and active **`Pilot courier delivery`** **ZAR 89.0** for exactly four locked **Gadgetgyz** pilot variants. **Safety:** checkout/payment/customer-access/public-launch still **BLOCKED**; **no** theme push/publish; **no** product data changed except shipping-profile assignment; **no** **`artifacts/`** committed; **`zadropshipping/`** untracked/uncommitted. **Notes:** initial create attempt required **ZA** province coverage; the successful mutation used **`includeAllProvinces: true`**; **`markets`** read access was unavailable in the local auth so market visibility was inferred from the shipping-zone context and blocked customer-access posture. Prior **Slice 21S** / **21R** / **21Q-B3** / **21Q-B1** / **21P** / **21O** / **21N** / **21M** / **21L** / **21K** / **21J**; **Slice 19B** preview posture unchanged.
- Tracker status: **Slice 21T PASS WITH NOTES** (**bounded Admin shipping execution completed; checkout/payment/customer access still blocked**) + **Slice 21S PASS WITH NOTES** (**readiness-only**) + **Slice 21R PASS WITH NOTES** + **Slice 21Q-B3 PASS WITH NOTES** + **Slice 21Q-B1** (**planning recorded**; **§15A**; **no** Admin) + **Slice 21P** (**planning recorded**; **§15**; **no** Admin) + **Slice 21O PASS WITH NOTES** (**§14**) + **Slice 21N PASS WITH NOTES** (**§13**) + **Slice 21M** (**§12**) + **Slice 21L PASS WITH NOTES** + **Slice 21K PASS WITH NOTES** + **Slice 21J PASS WITH NOTES** + **Slice 19B PASS** + **Slice 19A PASS WITH NOTES** + **Slice 18F** … **18A–C** — storefront **`/search`** honesty intact — **customer access BLOCKED** — **Shopify** shipping now includes the bounded pilot-only/product-specific profile while checkout/payment remain blocked
...
- Documentation sync status: **current for `project-control` + pilot doc (§12–§15C) + LLD Slice 21T contract note** after **Slice 21T** execution recording; **catalogue docs unchanged** — shipping-contract/Admin state now recorded as an isolated pilot-only/product-specific profile; **no** theme/storefront/checkout/payment/customer-access enablement behaviour changed; prior **Slice 19B** preview-sharing posture unchanged
