...
- Active slice: **DevOps / Platform Engineer** — **Slice 21AR** publish-for-preview execution **BLOCKED** on missing **`write_publications`** CLI auth scope; **human action:** re-auth store with **`read_publications,write_publications`**, then rerun bounded execution. **Product Owner** approved tag set and scope remain valid. **CJ** rows remain **DRAFT** / unpublished after rollback. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **app install/import/sync** **BLOCKED**. Continue **Slice 19B** bounded private-preview posture (**`## Slice 19B`**)
- Completed slice: **Slice 21AR** — **CJ** **3**-**SKU** publish-for-preview execution — **BLOCKED** — **DevOps** (**theme gate PASS**; **pre-snapshot PASS**; **publication step blocked**). **Method:** **`shopify store execute`**; stored auth scopes **`read_products`**, **`write_products`** only — **`publishablePublish`** requires **`write_publications`**. **Probe + rollback:** one CJ row briefly set **ACTIVE** with approved tags, then restored **DRAFT** + pre-change tags. **No** Online Store publication; **no** storefront visibility widening. **Theme:** **`preview-only`** / **`price-to-confirm`** / disabled purchase UI confirmed before mutation attempt. **Human action:** `shopify store auth --store dropshippoc.myshopify.com --scopes read_products,write_products,read_publications,write_publications`. **Execution note:** **`docs/pilot/slice-21ar-cj-publish-for-preview-execution.md`**. **Evidence (local; not committed):** **`artifacts/devops/slice-21ar-cj-publish-for-preview-execution/20260515-131150/`**. **LLD:** **unchanged** — no successful visibility change.
