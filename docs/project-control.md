...
- Active slice: **QA / Test Engineer** — validate cleaned **`controlled-pilot`** collection state after **Slice 21AL** **PASS** (**membership** + **draft**/**restricted** posture; **no** unintended public/checkout exposure). **Collection membership hygiene:** **resolved** in Admin. **Collection preview validation** remains a **separate** gate — **not** auto-approved. **Slice 21AH** / **21AG** / **21AF-D** CJ draft posture unchanged. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **app install/import/sync** **BLOCKED**. Continue **Slice 19B** bounded private-preview posture (**`## Slice 19B`**)
- Completed slice: **Slice 21AL** — bounded **`controlled-pilot`** collection membership cleanup — **PASS** — **DevOps / Platform Engineer** (**Product Owner** approved; bounded Admin execution after operator **`shopify store auth`**). **Method:** **`shopify store execute`** — pre-read (**7** members, all **DRAFT**, baseline match) → **`collectionRemoveProducts`** (four Gadgetgyz-era handles only) → post-verify (**3** CJ members; four removed rows still **DRAFT**, not in collection). **Kept in collection:** **CJYD230000901AZ** / **`beverage-bottle-oil-bottle-handle-holder`**; **CJYD211196101AZ** / **`usb-bag-sealer`**; **CJYD245830501AZ** / **`foldable-magnetic-phone-holder-desktop-tablet-stand`**. **Removed from collection only:** **`world-map-extended-mouse-pad`**, **`gizzu-usb-to-type-c-cable-2m`**, **`ugreen-4-in-1-usb-2-0-hub`**, **`acrylic-tablet-phone-stand`**. **No** publish/archive/delete/reject/exposure/media/pricing/claim/checkout/payment/customer-access changes. **Prior blocker:** commit **`28ef589`** (auth missing). **Cleanup note:** **`docs/devops/slice-21al-controlled-pilot-collection-cleanup.md`**. **Evidence (local; not committed):** **`artifacts/devops/slice-21al-controlled-pilot-collection-cleanup/20260515-131842/`**. **Next:** **QA / Test Engineer**. **LLD:** **unchanged**.
...
## Slice 21AL controlled-pilot collection cleanup (DevOps PASS)

- **Decision:** **Slice 21AL** is **PASS** — bounded **`controlled-pilot`** membership cleanup **executed** (**Product Owner** approved).
- **Pre-state:** **7** members (3 CJ + 4 Gadgetgyz-era), all **DRAFT** — matched **Slice 21AK** baseline.
- **Post-state:** **3** CJ members only; four older rows removed from collection, still **DRAFT**.
- **QA handoff:** validate cleaned collection state — membership, draft/restricted posture, no unintended public/checkout exposure. **Collection preview** remains separate gate.
- **Evidence (local; not committed):** **`artifacts/devops/slice-21al-controlled-pilot-collection-cleanup/20260515-131842/`**.
