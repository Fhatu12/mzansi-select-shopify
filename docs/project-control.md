...
- Active slice: Slice 13M docs-only **Neat Freak** supplier-path replacement (launch SV path closure; replacement routing) — this pass — pending Product Owner commit
- Next product proof slice / recommended next slice: **Slice 13N** — **`ZA Dropshipping`** app/admin **exact replacement product references** capture (see Slice 13M / **Slice 13N** bullets)
- Active owner: Product Manager
- Next owner: Product Owner — commit **Slice 13M** when approved; then **Slice 13N** (**ZA Dropshipping** refs), remaining **Gadgetgyz** acrylic gates, **Gadgetgyz** / **ZA** **`Cable Tidies Set`** proof as scoped, further preview QA, or launch-readiness planning as approved
- Last accepted slice: Slice 13M supplier-path replacement recommendation **accepted** as docs-only (pending commit); Slice 13L.1 tracker sync **committed** (`a33895a3bb242455c95da74a6c6f2c72bee946d6`); Slice 13L **Gadgetgyz** acrylic stand (`a87e8c6124d9708967dfbf2c6a208b601fe6f608`); Slice 13K.1 (`0054d66acfe926be21127ae02ba7453989c11131`); Slice 13K (`cb61ee28fbb3e575669236310d4e5b131fc697bf`); Slice 13J (`cef5713412ef218bee4af56ae9767c78d6304859`, **PASS WITH NOTES**)
- Last committed slice: Slice 13L.1 tracker sync (`a33895a3bb242455c95da74a6c6f2c72bee946d6`); Slice 13L **Gadgetgyz** proof closure docs (`a87e8c6124d9708967dfbf2c6a208b601fe6f608`); Slice 13K.1 (`0054d66acfe926be21127ae02ba7453989c11131`); Slice 13K (`cb61ee28fbb3e575669236310d4e5b131fc697bf`); docs tracker sync after Slice 13J (`353276375a3814cbe3ed929df5696bc60d7e03d5`); theme Slice 13J (`cef5713412ef218bee4af56ae9767c78d6304859`); prior documentation slice remains Slice 13I (`b61746cdc3a566bb3723b95581676da0f88b9561`)
...
- Current blockers:
  ...
  - **`ZA Dropshipping` (`Slice 13M`):** **primary replacement path** for **Home / Kitchen / storage-style** Neat‑Freak‑tied preview SKUs — Shopify integration, local-stock positioning, **no-inventory model**, fulfilment workflow, sample-order signal — **still blocked** by missing exact app/admin **product references**, **current costs**, **shipping fee**, **delivery-cost model**, **image usage**, **sample proof**, and downstream commercial gates
  - **`Ecomstock` (`Slice 13M`):** **backup supplier path** — SA wholesale/retail category positioning — **still blocked** by unclear **dropship workflow**, exact **product references**, **fulfilment process**, **shipping fee**, **image usage**, and **sample proof**
...
- Last tracker update: 2026-05-03 Slice 13M docs-only Neat Freak supplier-path replacement (pending commit)
- Tracker status: Updated (**Slice 13M** replacement routing and **Slice 13N** recommendation recorded; **`Neat Freak` launch SV path removed** — benchmark only)
- Catalogue plan status: Updated (**Slice 13M** recorded in readiness matrix and supplier matrix — pending commit)
- LLD status: **Unchanged in this pass** — Slice 13M is supplier/catalogue documentation only; no durable theme, rendering, architecture, or storefront behaviour change

## Current active pass

Slice 13M docs-only **Neat Freak** supplier-path replacement — `docs/project-control.md`, `docs/catalogue/mzansi-select-25-product-readiness-v1.md`, `docs/catalogue/local-supplier-sourcing-matrix-v1.md` only; no code, no Shopify Admin actions, no commit until Product Owner approves; **recommended next slice** **Slice 13N** — **`ZA Dropshipping`** app/admin **replacement product references** capture
...
- **Per-handle decisions (`Slice 13K` baseline + `Slice 13M` supplier-path replacement):** preview-only staged; **none** **`Supplier verified`**; **do not reject** concepts:
  1. **`Sink Strainer — Stainless Steel`** (`sink-strainer-stainless-steel`) — **`Slice 13M`:** replace Neat Freak with **`ZA Dropshipping` first**, **`Ecomstock` backup** — **stay `Supplier proof in progress` / preview-only**.
  2. **`Compact Organiser Basket`** — **`ZA Dropshipping` first**, **`Ecomstock` backup** — **`Supplier proof in progress` / preview-only**.
  3. **`Mini Plastic Divider Basket`** — **`ZA Dropshipping` first**, **`Ecomstock` backup** — **`Supplier proof in progress` / preview-only**.
  4. **`Cable Tidies Set`** — **`Gadgetgyz` first** (**stronger public dropship** than **Ecomstock**); **`ZA Dropshipping` backup** — **narrow exact item** remains required — **`Supplier proof in progress` / preview-only**.
...
- **Commercial / fulfilment / image-rights summary (`Slice 13M`):** **`Neat Freak` launch SV closure** redirects proof work to **`ZA Dropshipping`** / **`Ecomstock`** / **`Gadgetgyz`** lanes; **`Gadgetgyz`** acrylic stand retains **Slice 13L** blockers; **`ZA`** primary replacement path carries app/admin/reference/cost/shipping/sample gaps (**Slice 13N** captures **ZA** refs first). Product copy stays **conservative and factual**.
...
- **Product-by-product routing:** each row **stays `Supplier proof in progress` / preview-only staged**; **do not reject** concepts; **none** ready for **`Supplier verified` review**:
  1. **`Sink Strainer — Stainless Steel`** — **`ZA Dropshipping` first**, **`Ecomstock` backup**.
  2. **`Compact Organiser Basket`** — **`ZA Dropshipping` first**, **`Ecomstock` backup**.
  3. **`Mini Plastic Divider Basket`** — **`ZA Dropshipping` first**, **`Ecomstock` backup**.
  4. **`Cable Tidies Set`** — **`Gadgetgyz` first**, **`ZA Dropshipping` backup** (exact SKU still required).
...
- **Recommended next slice — `Slice 13N`:** **`ZA Dropshipping`** app/admin **exact replacement product reference capture** for **`Sink Strainer — Stainless Steel`**, **`Compact Organiser Basket`**, **`Mini Plastic Divider Basket`**, and **`Cable Tidies Set`** as **ZA backup if available**.
- **`Gadgetgyz` Cable Tidies exact item capture:** **optional only** if Product Owner includes it in **Slice 13N** scope — otherwise **keep** as a **separate focused slice**.
