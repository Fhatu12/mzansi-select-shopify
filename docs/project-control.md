...
- Active slice: **Slice 15F** — mobile overflow root-cause remediation (**local theme/docs fix complete; awaiting QA rerun / Slice 15A re-enable decision**)
...
- Next owner: **QA / Test Engineer** for **Slice 15G — mobile readiness rerun** and **Slice 15A re-enable decision**; Product Owner for follow-on supplier-proof or storefront prioritisation after QA outcome
...
- Documentation sync status: **current for `project-control` + `docs/lld`** after **Slice 15F** mobile overflow root-cause remediation sync (**this pass**); **`docs/catalogue`** **not edited**
- LLD status: **Slice 15F** responsive/mobile containment rules now extend prior **Slice 15C** guardrails with zero-min grid tracks, shrink-safe media wrappers, and intrinsic-width containment for promo / product / cart media; **`docs/catalogue`** unchanged

## Current active pass

**Theme + docs (this pass):** **Slice 15F** mobile overflow root-cause remediation — remove persistent page-level horizontal overflow across homepage, collection, PDP, search, and cart mobile/tablet routes by fixing intrinsic-width containment in promo/product/cart media while preserving north-star fidelity, **Slice 14B / 14C / 14D**, and preview-only safety behaviour; update **`docs/project-control.md`** + **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**; keep **`docs/catalogue`** untouched.
