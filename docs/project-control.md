...
- Documentation sync status: **current for `project-control` + `docs/lld`** after **Slice 15C** mobile overflow remediation + north-star fidelity preservation sync (**this pass**); **`docs/catalogue`** **not edited**
- LLD status: **Slice 15C** responsive/mobile fidelity rules now documented alongside prior **Slice 14B / 14D / 14C** contracts; **north-star stripping prevention** and targeted overflow-remediation guardrails added; **`docs/catalogue`** unchanged

## Current active pass

**Theme + docs (this pass):** **Slice 15C** mobile overflow remediation + north-star fidelity preservation — restore approved mobile/desktop visual direction from **`mzansi-select-mobile.html`** + **`mzansi-select-theme.html`**, fix only true page-level overflow sources, preserve **Slice 14B / 14C / 14D** and preview-only safety behaviour, update **`docs/project-control.md`** + **`docs/lld/mzansi-select-shopify-mvp-theme-v1.md`**, and keep **`docs/catalogue`** untouched.
