...
- Active slice: **DevOps / Platform Engineer** — **Slice 21AR** publish-for-preview execution **BLOCKED** on missing **`write_inventory`** CLI auth scope (**`availableForSale`** validation failed after successful publication mutations; **rollback complete**). **Human action:** re-auth store with **`read_inventory,write_inventory`** in addition to product/publication scopes, then rerun bounded execution. **Product Owner** approved tag set remains valid. **CJ** rows remain **DRAFT** / unpublished after rollback. **Headline gates** unchanged — **customer access** **BLOCKED**; **checkout/payment** **BLOCKED**; **public launch** **BLOCKED**; **`Supplier verified`** **BLOCKED**; **final pricing / delivery / product claims** **BLOCKED**; **app install/import/sync** **BLOCKED**. Continue **Slice 19B** bounded private-preview posture (**`## Slice 19B`**)
- Completed slice: **Slice 21AR** — **CJ** **3**-**SKU** publish-for-preview execution — **BLOCKED** — **DevOps** (**theme gate PASS**; **pre-snapshot PASS**; **retry:** publication scopes **PASS**; **`productUpdate`** + **`publishablePublish`** succeeded for all **3**; **`availableForSale: false`** validation **FAIL** — **`inventoryItem.tracked: false`**; **`inventoryItemUpdate`** blocked on **`write_inventory`**; **full rollback** to **DRAFT** / unpublished / pre-change tags). **Method:** **`shopify store execute`**. **Stored auth (retry):** **`read_products`**, **`write_products`**, **`read_publications`**, **`write_publications`** — **not** **`write_inventory`**. **No** durable Online Store publication after rollback. **Theme:** **`preview-only`** / **`price-to-confirm`** / disabled purchase UI confirmed. **Human action:** add **`read_inventory,write_inventory`** to CLI auth, then rerun. **Execution note:** **`docs/pilot/slice-21ar-cj-publish-for-preview-execution.md`**. **Evidence (local; not committed):** **`artifacts/devops/slice-21ar-cj-publish-for-preview-execution/20260515-132109-retry/`** (+ first attempt **`20260515-131150/`**). **LLD:** **unchanged**.
