...
- Authenticated **Slice 21AR** QA then surfaced one extra durable cautious-route contract: if the password-unlocked request path is exactly **`/collections/controlled-pilot`** and Shopify resolves that preview route through the 404 template, the theme may recover the intended controlled-preview browse surface from an explicit allowlist of exactly the three approved CJ handles via `all_products[...]`, rather than depending on `collections['controlled-pilot']` or leaving the user on a generic 404 recovery shell.
