...
- **Slice 21J:** Bounded **implementation planning** for the **Gadgetgyz-only** four-SKU pilot (order cap **10**, pilot delivery **supplier evidence** per **Slice 21K** — observed **R89** Economy + checkout-displayed **3–5 business days** as **supplier signals only**, prior **R99** assumption withdrawn as confirmed, basket/setup questions **open**, pilot-only pricing, manual fulfilment SOP add-ons, checkout/payment **readiness checks without enablement**, QA and DevOps checklists) — recorded in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** and the **Slice 21J** section of **`docs/catalogue/mzansi-select-25-product-readiness-v1.md`**. **Product Owner operational SLAs** (stock-failure/refund, customer updates, evidence retention/deletion, **Gadgetgyz** delivery gate before customer access, Admin/staging/checkout deferral) are **canonical** in the **pilot doc** and **`docs/project-control.md` `## Slice 21J`** / **`## Slice 21K`**.
- **Theme contract:** until a later approved implementation slice, storefront behaviour remains as implemented under **Slice 21E** (`controlled_pilot_trust_mode`, pilot PDP/cart/search/footer/card copy); **Slice 21J** / **21K** do **not** by themselves change Liquid, Admin, or checkout state.
...
- **Operational checkout / customer-data constraints (theme + docs discipline):** do **not** embed real customer, order, payment, or PII examples in theme copy, fixtures, or **repository** documentation; any future checkout/cart UI must stay **honest** about what is **not** enabled until explicit approval; pilot delivery fee/timing lines remain **non-final** supplier-planning signals (**R89** / **3–5 business days** from **Slice 21K** — **not** guaranteed promises) until basket/setup confirmation closes; align evidence handling with **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §7.3** — approved systems of record only (e.g. Shopify / payment / order records), not repo or local project notes for PII.
