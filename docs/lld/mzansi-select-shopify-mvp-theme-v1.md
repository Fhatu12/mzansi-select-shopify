...
- **Slice 21CR:** announcement topbar uses a centred static strip on viewports **`> 1080px`** and a north-star-style right-to-left ticker (`topbar-track` + duplicated items, **`-50%`** keyframe loop) on viewports **`≤ 1080px`**. **`prefers-reduced-motion: reduce`** disables the ticker animation and keeps horizontal scroll inside the bounded topbar rail. Hero collage mobile rail scroll must stay inside **`hero-visual`** (`width/max-width: 100%`, `overflow: hidden` on the visual shell; `width/max-width: 100%` on **`.collage-grid`**). **`main.page-shell`** may use **`overflow-x: clip`** as a guard only after rail containment is applied. A global **back-to-top** button appears after **400px** scroll, scrolls to the document top, returns focus to **`#MainContent`**, and sits above the mobile bottom bar (**`bottom: calc(var(--bottom-bar) + 16px)`**).
- **Slice 21CU:** on viewports **`≤ 1080px`**, the ticker **`.topbar-track`** is **`position: absolute`** inside a **`width: 100%`** **`.topbar-marquee`** viewport with fixed height and **`overflow: hidden`**, so the duplicated track width does not inflate **`documentElement.scrollWidth`**. **`prefers-reduced-motion: reduce`** restores **`position: static`** on the track and keeps manual scroll inside the marquee only.
- **Slice 21DC:** on viewports **`≤ 900px`**, the homepage **category strip** must scroll only inside **`.cat-strip`** (**`overflow-x: auto`**, hidden scrollbar, **`contain: layout style`**). **`.cat-strip-inner`** carries the max-content flex rail (**`display: flex`**, **`width: max-content`**, **`overflow-x: hidden`**). **`.cat-strip-track`** uses **`display: contents`** so department tiles participate in the inner flex row without contributing an extra in-flow wide box to root **`documentElement.scrollWidth`**. Desktop/wide viewports keep **`.cat-strip-inner`** as the scroll viewport per the base stylesheet.
...
- Homepage and global navigation chrome must avoid speed, coverage, or readiness claims such as **`Fast Delivery`** or **`South Africa Wide`** when fulfilment remains under review; use neutral review-state wording instead (for example **`Delivery review`** and **`South Africa only`**).
...
- **`sections/primary-navigation.liquid`**: mobile drawer **Sign in** becomes a **`button`**, **`type="button"`**, **`disabled`**, **`aria-disabled="true"`**, visible label **Customer accounts paused**; bottom-bar **Account** becomes **`bb-item bb-item--deferred`** (**no** account route). **Slice 21BI** further pauses shared-shell cart entry points in the drawer and bottom bar as disabled/deferred chrome while cart and checkout remain blocked.
...
**Slice 21CU** (2026-05-21): Topbar ticker overflow containment — **`assets/theme.css`** only; absolute-positioned **`.topbar-track`** inside clipped **`.topbar-marquee`** so root-level mobile homepage horizontal overflow from the duplicated ticker width is removed without disabling ticker animation. **No** commerce, Admin, publish, or API enablement.

**Slice 21CR** (2026-05-20): Mobile UX polish — homepage horizontal overflow remediated by constraining the hero collage rail inside **`hero-visual`**; announcement topbar ticker on **`≤1080px`** viewports with reduced-motion fallback; accessible **back-to-top** control in **`layout/theme.liquid`**. Preview-safe copy unchanged (**Checkout preview**, no live payment implication). **No** commerce, Admin, publish, or API enablement.
...
## Slice 21I through Slice 21T — Gadgetgyz controlled pilot planning and bounded shipping execution
...
- **Slice 21J:** Bounded **implementation planning** for the **Gadgetgyz-only** four-SKU pilot (order cap **10**, pilot delivery **Slice 21K** evidence + **Slice 21L** wording/fee posture, **R89** as **current planning signal** only, **no** **3–5 business days** in customer-facing copy, pilot-only pricing, manual fulfilment SOP add-ons, checkout/payment **readiness checks without enablement**, QA and DevOps checklists) — recorded in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** and the **Slice 21J** section of **`docs/catalogue/mzansi-select-25-product-readiness-v1.md`**. **Product Owner operational SLAs** are **canonical** in the **pilot doc** and **`docs/project-control.md` `## Slice 21J`** … **`## Slice 21P`**.
- **Slice 21L:** Delivery wording + fee posture — **PASS WITH NOTES**; **Shopify** shipping/checkout **not configured** in docs passes — bounded planning **§6A** / blockers **§6B** in pilot doc.
- **Slice 21M:** Bounded Shopify shipping/checkout **Admin configuration plan** — **planning only** (**pilot §12**); **no** Admin execution until **Product Owner** accepts **§12**; **Shopify** shipping/checkout **unconfigured** until a later execution slice.
- **Slice 21N:** **Product Owner** accepts execution-readiness as **PASS WITH NOTES** — **`docs/pilot/...` §13**; **CONDITIONAL GO** for future bounded Admin shipping **only after** **§15 (`Slice 21P`)** prerequisites: **Shopify** rows **verified**; **reuse/update vs create-new** **per-SKU Product Owner** decisions; **no duplicate rows**; pilot handles locked — **general-profile fallback not approved** unless separately approved; **South Africa only**; checkout label **Pilot courier delivery — R89** **only** for controlled pilot checkout testing when later approved.
- **Slice 21O:** **Product-staging readiness** — **PASS WITH NOTES** — **§14**; **no** Admin/import/staging approval from acceptance text alone.
- **Slice 21P:** Bounded **Shopify Admin product-staging implementation plan** — **§15**; **planning only** — **verify existing rows** before later writes; **reuse/update only** after **separate PO** approval per SKU; **no duplicate rows**; **product-specific shipping profile** dependency documented in pilot **§15** / **project-control `## Slice 21P`**.
- **Slice 21T:** Bounded Shopify Admin shipping-profile execution — **PASS WITH NOTES** — exact isolated profile **`Mzansi Select Controlled Pilot — Gadgetgyz`** now exists with **South Africa** (**ZA**) and active **`Pilot courier delivery`** at **ZAR 89.0** for exactly four locked **Gadgetgyz** draft pilot variants (**`DP0402`**, **`CR106-20277`**, **`GCPU2C2`**, **`74886`**); default **`General profile`** remains present with **`Standard shipping`** **ZAR 99.0** for non-pilot scope; checkout/payment/customer access/public launch remain blocked and no theme behaviour changed.
- **Theme contract:** until a later approved implementation slice, storefront behaviour remains as implemented under **Slice 21E** (`controlled_pilot_trust_mode`, pilot PDP/cart/search/footer/card copy); **Slice 21J** / **21K** / **21L** / **21M** / **21N** / **21O** / **21P** do **not** by themselves change Liquid, Admin, or checkout state.
- **Theme contract after Slice 21T:** Liquid/theme behaviour remains unchanged; **Slice 21T** is an Admin-only shipping-profile change and does **not** by itself expose checkout, payment, customer access, or public launch.
- **Pilot-specific delivery/returns/support strings** for go-live copy are **canonical** in the pilot doc **§1** (**Slice 21L**).
- **Bounded Shopify shipping / checkout contract after Slice 21T:** the Admin shipping profile is now configured, but this document still does **not** authorize checkout, payment, or customer-access enablement. The active bounded contract is: exact profile **`Mzansi Select Controlled Pilot — Gadgetgyz`**; exact visible pilot line **`Pilot courier delivery`** at **R89**; exact product scope limited to the four locked **Gadgetgyz** draft pilot variants; **South Africa** only; **General profile** remains intact for non-pilot scope. **R89** remains a controlled-pilot planning/test rate until supplier re-check + checkout QA + Product Owner go/no-go confirm it. **Never** surface supplier **3–5 business days** as customer-facing copy. Required future checks still apply: preview/test checkout; **mobile** + **desktop** checkout QA; payment readiness; delivery fee visibility; refund/cancellation path; customer support/contact path; **Product Owner** final pilot go/no-go — see **pilot §6A**, **§12**, **§13**, **§14**, **§15**, **§15C**.
- **Operational checkout / customer-data constraints (theme + docs discipline):** do **not** embed real customer, order, payment, or PII examples in theme copy, fixtures, or **repository** documentation; any future checkout/cart UI must stay **honest** about what is **not** enabled until explicit approval; align evidence handling with **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §7.3** — approved systems of record only (e.g. Shopify / payment / order records), not repo or local project notes for PII.
...
- For `preview-only` products, catalog media is suppressed in favour of theme placeholders unless the `image-permission-confirmed` tag is present (Product Owner evidence required before using this tag); the three Slice 21AR CJ controlled-preview handles are stricter and always keep generic placeholder treatment while they remain in this imported-supplier preview scope, even if preview-tag parsing is imperfect on the staged rows.
...
- Authenticated **Slice 21AR** QA then surfaced one extra durable cautious-route contract: if Shopify resolves **`/collections/controlled-pilot`** through the 404 template, the theme should still identify that route from stable URL signals such as `request.path` or `canonical_url`, then recover the intended controlled-preview browse surface from an explicit three-item static preview-safe card set. That avoids dependence on `collections['controlled-pilot']`, `all_products[...]`, or other product-object hydration behavior on 404 pages while still emitting deterministic QA-visible signals for the approved CJ handles only.
- A later post-parity rerun showed those two URL signals may still be unavailable on the rendered 404 page for `/collections/controlled-pilot`. Unless a broader route contract is approved, the remaining safe theme posture is to avoid turning every 404 into the controlled-pilot shell; Product Owner direction is therefore required before widening the recovery condition further.
- The fixed-route QA harness now treats unloaded or broken product-surface `<img>` elements as no-media placeholder treatment rather than real product media, so broken gallery requests do not fail the CJ media gate unless an image actually loads.
- On `preview-route-cautious` surfaces (including the controlled-pilot recovery route above), cart chrome should render as honest deferred/non-clickable UI rather than linking to **`routes.cart_url`**. This keeps preview-only collection/PDP surfaces aligned with the accepted non-purchasable controlled-pilot posture without enabling cart, checkout, payment, customer accounts, or wishlist persistence.
