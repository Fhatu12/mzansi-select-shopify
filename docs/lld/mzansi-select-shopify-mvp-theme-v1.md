...
- **Slice 21J:** Bounded **implementation planning** for the **Gadgetgyz-only** four-SKU pilot (order cap **10**, **R99** pilot delivery fee **assumption** pending Gadgetgyz checkout confirmation, pilot-only pricing, manual fulfilment SOP add-ons, checkout/payment **readiness checks without enablement**, QA and DevOps checklists) — recorded in **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md`** and the **Slice 21J** section of **`docs/catalogue/mzansi-select-25-product-readiness-v1.md`**. **Product Owner operational SLAs** (stock-failure/refund, customer updates, evidence retention/deletion, **R99** confirmation gate, Admin/staging/checkout deferral) are **canonical** in the **pilot doc** and **`docs/project-control.md` `## Slice 21J`**.
...
- **Operational checkout / customer-data constraints (theme + docs discipline):** do **not** embed real customer, order, payment, or PII examples in theme copy, fixtures, or **repository** documentation; any future checkout/cart UI must stay **honest** about what is **not** enabled until explicit approval; **R99** and delivery lines remain **pilot assumption / non-final** in customer-facing surfaces until the **separate Gadgetgyz mechanism confirmation pass** closes; align evidence handling with **`docs/pilot/mzansi-select-controlled-live-pilot-v1.md` §7.3** — approved systems of record only (e.g. Shopify / payment / order records), not repo or local project notes for PII.
